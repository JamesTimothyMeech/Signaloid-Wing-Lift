...
Let $v$ be the velocity of the air (m/s) at a specific point in the flow ($\mathrm{m/s}$) and $v_\infty$ be the uniform velocity (m/s) of the air upstream from the airfoil ($\mathrm{m/s}$).
...
Perhaps this should not be too much of a suprise as we can write that $C_L = \sum_{i=0}^{N-1} (C_p^\mathrm{Lower}[i] - C_p^\mathrm{Upper}[i]) \Delta x$ where $N$ is the number of $C_p$ measurements and $\Delta x$ is the distance between $C_p$ measurements that we assume to be constant for simplicity. 
